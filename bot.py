...
# Start health check server in background thread for 24/7 hosting platforms (Render, Railway, etc.)
...
def calculate_selling_cents(cost_cents, product_id=None):
    ...
    if product_id is not None:
        ...
        if p_id_str in custom_prices:
            # Custom price given in Dollars/Units -> convert to cents
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 4. Deposit Instructions
        ...
        supp_user = config.SUPPORT_USERNAME.replace("@", "")
        text = (
            f"💳 <b>Deposit Payment Details:</b>\n\n"
            f"• <b>Easypaisa / JazzCash:</b> <code>03001234567</code>\n"
            f"• <b>USDT (TRC-20):</b> <code>TYOUR_CRYPTO_WALLET_ADDRESS</code>\n"
            f"• <b>Bank Transfer:</b> <code>Bank Details Here</code>\n\n"
            f"⚠️ <b>Important:</b> Payment send karne ke baad screenshot aur User ID support ko bhejein.\n\n"
            f"🆔 <b>Your User ID:</b> <code>{user_id}</code>\n"
            f"👤 Send screenshot to: @{escape(supp_user)}"
        )
        keyboard = [
            [InlineKeyboardButton("💬 Contact Support", url=f"https://t.me/{supp_user}")],
            [InlineKeyboardButton("🔙 Back", callback_data="main_menu")]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
    ...
    # 5. Products List with Pagination
        ...
        if not success or not isinstance(products, list):
            err_msg = products.get("error", "API Connection Error") if isinstance(products, dict) else "Unknown Error"
            await query.edit_message_text(
                f"⚠️ <b>Products fetch nahi ho sake:</b>\n<code>{escape(err_msg)}</code>\n\n"
                "Please verify your Qamify API Key.",
        ...
        if not products:
            await query.edit_message_text(
                "📦 Filhal store mein koi products available nahi hain.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="main_menu")]]),
                parse_mode="HTML"
            )
            return
    ...
    # 6. View Single Product
        ...
        if not p:
            await query.edit_message_text("❌ Product information nahi mil saki.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="products_page_0")]]))
            return
        ...
        desc = escape(p.get("description") or "Instant delivery digital product key upon purchase.")
    ...
    # 7. Buy Product Confirmation & Execution
        ...
        if not deducted:
            await query.edit_message_text("❌ Balance deduction failed. Please try again.")
            return

        await query.edit_message_text("⏳ <b>Purchasing license key from server, please wait...</b>", parse_mode="HTML")
        ...
        if order_success:
            ...
            # REFUND balance back to user if API failed
            ...
            await query.edit_message_text(fail_text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
...
def main():
    ...
    print("🚀 Initializing Qamify Reseller Telegram Bot...")
    
    builder = ApplicationBuilder().token(config.TELEGRAM_BOT_TOKEN)
    
    # If proxy configured in .env / config
    if config.PROXY_URL:
